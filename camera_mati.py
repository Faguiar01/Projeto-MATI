...
## Como a ROI "recorta" o frame na memória
```
FRAME COMPLETO (640 x 480 pixels)
┌─────────────────────────────────────┐
│                                     │
│         ┌─────────────┐             │
│         │  ROSTO      │  ← (x, y)  │
...
roi_gray   = frame_cinza [y:y+h, x:x+w]  ← entrada para detectar olhos
roi_color  = frame       [y:y+h, x:x+w]  ← saída para desenhar círculos

As duas ROIs apontam para a MESMA memória do frame.
Desenhar em roi_color = desenhar no frame original automaticamente.
```
...
┌──────────────────────────────────────┐
│ Olhos Detectados: 2                  │
│ ATENCAO: OLHOS NAO DETECTADOS  ←(só aparece se olhos=0 com rosto visível)
│                                      │
│        ┌──────────────┐              │
│        │   ⊙      ⊙   │  ← círculos azuis nos olhos
│        │              │  ← retângulo verde no rosto
│        └──────────────┘              │
└──────────────────────────────────────┘